    ...
    if name == "query_document":
        ...
        try:
            ...
            if not collection:
                return [TextContent(
                    type="text",
                    text="Error: ChromaDB collection is not initialized. Please run chroma_db.py first."
                )]
        ...
        try:
            ...
            return [TextContent(
                type='text',
                text=f"Collection name: bchpdf_collection\nNumber of documents: {count}"
            )]
        except Exception as e:
...
async def handle_list_resources() -> list[Resource]:
    """List all the available document resources"""
    resources = []
    try:
...
# ===========================================================
# Handling reading PDF resources
# ===========================================================
...
async def handle_read_resource(uri: str):
    ...
    try:
        ...
        #Combine all pages into one test page markers
        full_text = ""
        for i, page in enumerate(pages):
            full_text = f"\n\n--- Page {i+1} ---\n\n"
            full_text += page.page_content
        logger.info(f"Loaded Document {document_name} with {len(pages)}. Preview: {full_text[:200]}...")
        return full_text
    except Exception as e:
...
async def handle_get_prompt(name: str, arguments: dict[str, str] | None) -> GetPromptResult:
    ...
    if name  == "deep_analysis":
        ...
        query = arguments.get("query", "main_theme")
            ...
            messages=[
                ...
                PromptMessage(
                    ...
                    content=TextContent(
                        ...
                        - Connections to the broader context if applicable

                        Apply the above the key points where necessary and applicable.
                        format your analysis in a well-structured mannner with clear headings and concise explanations
                        """
                    )
                )
            ]
